...
function Library() {
  const myLibrary = []
  const addBookToLibrary = (title, author, pages, read, image, desc) => {
    const id = crypto.randomUUID()
    const newTitle = new Book(title, author, id, pages, read, image, desc)
    if (
      myLibrary.find(
        (title) =>
          newTitle.title + newTitle.author === title.title + title.author
      )
    ) {
      return
    } else {
...
;(function Dashboard() {
  ...
  const displayBooks = () => {
    for (const [i, book = value] of library.getLibrary().entries()) {
      const getAllTitles = document.querySelectorAll('.title')
      const allTitles = Array.from(getAllTitles)
      const sameTitle = allTitles.find(
        (title) => title.textContent == book.title
      )
  ...
  function removeBook(id) {
    ...
    target.remove()
    const index = library.getLibrary().findIndex((book) => book.id === id)
